...
export const Route = createFileRoute("/admin/_admin/users")({ component: UsersPage });
...
function UserFormDialog({ user }: { user?: UserAccount }) {
  ...
  const {
    ...
    defaultValues: user
      ...
      : { tag: "", username: "", name: "", email: "", phone: "", password: "", appRoleIds: [] as number[] },
  });
    ...
    if (isEdit) {
      ...
      createUser.mutate(
        ...
        },
        {
          onSuccess: () => {
            setOpen(false);
            reset({ tag: "", username: "", name: "", email: "", phone: "", password: "", appRoleIds: [] });
